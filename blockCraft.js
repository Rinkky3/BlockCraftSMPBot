...
//const Streaming = require("discord-streaming"); //npm i discord-streaming
bot.privates = require('../privates.json')
...
bot.queue = new Map();
const youtube = new YouTube(bot.privates.ytapi)
let stopping = false
bot.stopping = false;
var voteSkipPass = 0;
var voted = 0;
...
const requestOpts = {
	hostname: "mcapi.us",
    path: "/server/status?ip=66.23.200.119&port=25565",
	method: 'GET'
} 
...
//   let ivc = await newMember.guild.roles.find(r => r.name == "In Voice call");
...
bot.on('guildMemberAdd', member => {
    ...
    const channel = member.guild.channels.find(c => c.name == 'newcomers-and-applications');
    const channelinfo = member.guild.channels.find(c => c.name == "info");
    // Do nothing if the channel wasn't found on this server
    if (!channel) return;
    // Send the message, mentioning the member
    channel.send(`Welcome ${member}! You can apply to get whitelisted, by clicking the link provided here: ${channelinfo}. Each answer must be 3 lines minimum. Good luck!`); //You can apply to get whitelisted, by clicking the link provided here: ${channelinfo}. Your answers must be a paragraph long. Good luck! 
...
Streaming(bot, {
    live :  "LIVE NOW"
    ,required : "Player" // optional parameter, only use if you want to take action on people of a specific role
});*/
...
bot.on('message', async message => {
    ...
    //json stuff
    if (!bot.userData[message.author.id]){ 
